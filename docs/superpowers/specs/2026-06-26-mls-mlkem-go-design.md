...
> **Refinement (2026-06-28, validated by the metalnet simulation — see `docs/superpowers/specs/2026-06-28-metalnet-simulation-design.md`, esp. Appendix A).** B1 as written assumes a **strongly-consistent lease store** (etcd / K8s), which **real metalbond does not have** — it is an independent route-reflector *pair* with no consensus and no shared store. Two store-free models are recommended instead, both built on **per-reflector local** accept-once registers (`MemorySequencer`, never shared): (a) **dual-group pure redundancy** — each VNI runs *two* independent single-sequencer groups, one per reflector, with dual ESP SAs. This is the model the simulation **implements and validates**: it never forks (each replica is serialized by one local register) and gives the strongest guarantee — **zero data-plane packet loss** even with a whole reflector down/partitioned — at ~2× control-plane cost. (b) **Static-precedence local registers** — *one* group per VNI, clients pick the authoritative reflector by static per-VNI precedence (`vni % numDS`) and fall back only when the primary is unreachable; fork-free except for a *partitioned-but-alive* primary, healed by §5.6 recovery to the precedence-authoritative branch, at ~1× cost (a deferred scaling-oriented alternative). The leased/CP variant (above) remains correct **iff** a consistent store is actually available. **Recommended default for metalbond: dual-group redundancy (validated) — never the leased CP store.**
