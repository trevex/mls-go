...
> **Refinement (2026-06-28, validated by the metalnet simulation — see `docs/superpowers/specs/2026-06-28-metalnet-simulation-design.md` §9).** B1 as written assumes a **strongly-consistent lease store** (etcd / K8s), which **real metalbond does not have** — it is an independent route-reflector *pair* with no consensus and no shared store. The recommended default for metalbond-as-it-is is therefore **B1 *without* the lease/fence**: each reflector runs its **own local** accept-once register (`MemorySequencer`, never shared) and clients choose the authoritative reflector by **static per-VNI precedence** (`vni % numDS`), falling back to the standby only when the primary is unreachable. This is fork-free in the common case *and* under a clean primary crash; the **only** residual fork window is a *partitioned-but-alive* primary, healed deterministically by §5.6 recovery to the **precedence-authoritative branch** (no need to compare both commits, no reflector coordination). The leased/CP variant (above) remains the right choice **iff** a consistent store is actually available. **Default target for metalbond: static-precedence local registers.**
